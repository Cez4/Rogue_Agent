...
3. **A Armadilha do Respawn (Ghost Trail State Sync):** UIs de barras atrasadas (Trails) caem silenciosamente em desuso se apenas o valor principal da vida (`_current_fill`) for resetado no respawn. Quando a vida cura de 0.0 para 1.0 instantaneamente, a variável atrasada (`_current_trail`) fica travada em 0.0 e a condição de decaimento (`trail > fill`) nunca mais é satisfeita. **Regra:** Sempre implemente um "Snap" explícito da variável secundária para igualar à primária sempre que o valor subir bruscamente (cura/respawn). Esta regra é vital para futuras Orbs de Stamina e Mana.
