...
1. `Cliente/nexus/docs/plano-sprint-combat-clash-parry-v1-2026-05-12.md` e historico tecnico apos V9.
2. Combat Clash temporal foi auditado e removido do runtime; nao reativar como regra global automatica.
3. O custo de stamina do ataque interrompido ja e o primeiro "stamina damage" aprovado; nao duplicar punicao antes de QA.
4. Telemetria util preservada:
...
6. Se criar `DefenseComponent`/`ParryComponent`, ele deve ser plug-and-play como `KnockbackComponent` e `HitReactionComponent`.
7. Parry futuro deve ser data-driven por chance/atributo/custo/cooldown em profile `.tres`, consultado antes de dano/knockback/Hit Reaction.
8. Nao adicionar exports de Parry/Defense em `Actor8DirLimbo`.
9. Nao editar `.tscn`/`.tres` por texto; qualquer componente/profile em cena deve ser aplicado via Godot/editor API.
10. Validacao MCP obrigatoria antes de commit: `open_scene -> play_scene -> get_godot_errors -> conferir telemetria`.
...
## Sequenciamento Inventory / ExpressoBits V12-V14
1. `Cliente/nexus/docs/status-freeze-funcional-v12-inventory-expresso-spike-2026-05-13.md` congelou bridge/authority/craft.
2. `Cliente/nexus/docs/status-freeze-funcional-v13-inventory-datadriven-core-2026-05-13.md` congelou a database ExpressoBits como fonte oficial de equipamento do Player.
3. `Cliente/nexus/docs/status-freeze-funcional-v14-dynamic-loot-dex-2026-05-13.md` e o estado atual para loot dinamico, DEX e ponte runtime de combate.
4. O addon core e `res://addons/inventory-system/`.
5. `res://addons/inventory-system-demos/` e somente referencia de estudo/consulta e nao deve virar runtime do projeto.
6. A database oficial e `res://configs/items/inventory/nexus_inventory_database_v1.tres`.
7. Nao copiar `CharacterInventorySystem` do demo para o projeto; manter bridge propria, pequena e autoritativa.
8. Cliente envia intent; host valida e executa `add/remove/transfer/equip/craft`.
9. O Player resolve equipamento pela ponte ExpressoBits -> `NexusEquipmentAdapter` -> `EquipmentLoadout`/`WeaponData`/`CombatActionData` em memoria.
10. Stamina, attack range, stop distance e kiting do Player devem consumir `actor.get_equipment_loadout_runtime()`, nao `actor.equipment_loadout` direto.
11. `ItemStack` seguro: `item_id`, `amount`, `properties`. Buscar definition com `inventory.database.get_item(item_id)`.
12. Nunca usar `stack.item`; nunca inserir stack via `.append()` em `inventory.stacks`.
13. Nao editar `.tres` estruturais por texto; usar editor visual do addon ou Godot/editor API.
14. Se houver colisao com classes nativas do addon, renomear nomes genericos nossos; exemplo aprovado: `Slot` -> `EquipmentSlot`.
15. Validacao minima por bloco: `get_godot_errors`, smoke de database/inventory/craft quando aplicavel, telemetria de intent/commit/reject e, se afetar combate, `attack_stamina_cost.required` + `kiting_started/holding`.
