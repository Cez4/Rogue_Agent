...
static func should_start_wander(actor: Actor8DirLimbo, delta: float) -> bool:
	if not ActorSocialProfileRuntime.enable_wander(actor) or actor.player_controlled:
		return false
...
static func try_play_wander_emote(actor: Actor8DirLimbo) -> void:
	...
	if now_sec < ActorRuntimeBridgeRef.get_next_wander_emote_allowed(actor):
		return
	if randf() > clampf(ActorSocialProfileRuntime.wander_emote_chance(actor), 0.0, 1.0):
	...
	await ActorSocialRuntimeRef.show_emote(
		actor,
		ActorSocialProfileRuntime.wander_emote_name(actor),
		true,
		maxf(0.2, ActorSocialProfileRuntime.wander_emote_hold_sec(actor)),
		1
	)
...
static func schedule_next_wander_emote(actor: Actor8DirLimbo) -> void:
	var now_sec: float = Time.get_ticks_msec() * 0.001
	var min_cd: float = maxf(0.0, ActorSocialProfileRuntime.wander_emote_min_cooldown_sec(actor))
	var max_cd: float = maxf(min_cd, ActorSocialProfileRuntime.wander_emote_max_cooldown_sec(actor))
...
static func pick_random_wander_target(actor: Actor8DirLimbo) -> Vector2:
	...
	for _i in range(maxi(1, ActorSocialProfileRuntime.wander_max_attempts(actor))):
		var angle: float = randf() * TAU
		var min_radius: float = ActorSocialProfileRuntime.wander_radius_min(actor)
		var max_radius: float = maxf(min_radius, ActorSocialProfileRuntime.wander_radius_max(actor))
...
static func reset_wander_timer(actor: Actor8DirLimbo) -> void:
	var min_delay: float = ActorSocialProfileRuntime.wander_delay_min_sec(actor)
	var max_delay: float = maxf(min_delay, ActorSocialProfileRuntime.wander_delay_max_sec(actor))
	ActorRuntimeBridgeRef.set_next_wander_delay(actor, randf_range(min_delay, max_delay))
